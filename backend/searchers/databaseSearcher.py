...
def search_database(rawSearch, database, n=20):
    """
    ...
    if (len(searchTokens)==1):
        ...
        for result in unfilteredResults[0]:
            ...
            sortedResults = filteredResults.sort(cmp=(lambda result:result[-1]), reverse=True)
            # final results is list of url and title of sorted pages
            finalResults = [result[:2] for i, result in enumerate(sortedResults) if i<n]
    return finalResults
